...
func (suite *HandlerTestSuite) TestEnsureNativedexParams_DefaultsAlreadySet() {
	...
	if nativeDexAddr == zeroAddrBytes {
		params.VerifiedNativeDexAddress = verifiedNativeDexAddress
		needsUpdate = true
	}
	if crocPolicyAddr == zeroAddrBytes {
		params.VerifiedCrocPolicyAddress = verifiedCrocPolicyAddress
		needsUpdate = true
	}
	...
	suite.Require().Equal(verifiedNativeDexAddress, updatedParams.VerifiedNativeDexAddress)
	suite.Require().Equal(verifiedCrocPolicyAddress, updatedParams.VerifiedCrocPolicyAddress)
}
...
func (suite *HandlerTestSuite) TestEnsureNativedexParams_ParamsAlreadyConfigured() {
	...
	configuredParams := types.Params{
		VerifiedNativeDexAddress:     verifiedNativeDexAddress,
		VerifiedCrocPolicyAddress:    verifiedCrocPolicyAddress,
	...
	params, err := suite.app.NativedexKeeper.GetParamsIfSet(suite.ctx)
	suite.Require().NoError(err)
	suite.Require().Equal(verifiedNativeDexAddress, params.VerifiedNativeDexAddress)
	suite.Require().Equal(verifiedCrocPolicyAddress, params.VerifiedCrocPolicyAddress)
}
