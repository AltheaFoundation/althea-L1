...
// queryTokenizedAccountThresholds is used by the module control tokenized account balances, it calls the
...
func (k Keeper) queryTokenizedAccountThresholds(ctx sdk.Context, nftAddress common.Address) ([]types.TokenizedAccountThreshold, error) {
	...
	// The thresholds are returned as 2 dynamic arrays of 32 byte values. The contract should ensure the array lengths are the same
	// however that is checked here.
	// Dynamic values have a head value = offset where their info + elements begin. The info gives the number of elements in the array.
	// So we want to see [ addresses off. | amounts off | num addresses | addr 1 | ... | addr n | num amounts | amt 1 | ... | amt n ]
	thresholds := []types.TokenizedAccountThreshold{}
	thresholdBytes := res.Ret
	addressesInfoOffsetBz := thresholdBytes[0:32]                                // Index of num addresses as bytes
	amountsInfoOffsetBz := thresholdBytes[32:64]                                 // Index of num amounts as bytes
	addressesInfoOffset := big.NewInt(0).SetBytes(addressesInfoOffsetBz).Int64() // Index of num addresses
	amountsInfoOffset := big.NewInt(0).SetBytes(amountsInfoOffsetBz).Int64()     // Index of num amounts

	addressesInfoBz := thresholdBytes[addressesInfoOffset : addressesInfoOffset+32] // Num addresses as bytes
	amountsInfoBz := thresholdBytes[amountsInfoOffset : amountsInfoOffset+32]       // Num amounts as bytes
	addressesInfo := big.NewInt(0).SetBytes(addressesInfoBz).Int64()                // Num addresses
	amountsInfo := big.NewInt(0).SetBytes(amountsInfoBz).Int64()                    // Num amounts
	if addressesInfo != amountsInfo {
		return nil, sdkerrors.Wrap(types.ErrInvalidThresholds, "unexpected difference in the number of tokens and number of amounts")
	}

	for i := int64(0); i < addressesInfo; i++ {
		// The addresses start at 32 bytes past the info location
		addressBlockStart := addressesInfoOffset + 32 + (i * 32) // Start of this address's 32 byte block
		addressStart := addressBlockStart + 12                   // Addresses are actually 20 bytes, the block's first 12 bytes are all 0s
		addressEnd := addressStart + 20
		addressBz := thresholdBytes[addressStart:addressEnd]
		address := common.BytesToAddress(addressBz)

		// The amounts start at 32 bytes past the info location
		amountStart := amountsInfoOffset + 32 + (i * 32)
		amountEnd := amountStart + 32
		amountBz := thresholdBytes[amountStart:amountEnd]
		amount := big.NewInt(0).SetBytes(amountBz)

		thresholds = append(thresholds, types.NewTokenizedAccountThreshold(address, *amount))
	}
...
func (k Keeper) IsTokenizedAccount(ctx sdk.Context, account sdk.AccAddress) bool {
	isTokenized, entry := k.IsTokenizedAccountWithValue(ctx, account)
	k.Logger(ctx).Info("IsTokenizedAccount", "isTokenized", isTokenized, "entry", entry.String())
	return isTokenized
}
...
func (k Keeper) IsTokenizedAccountWithValue(ctx sdk.Context, account sdk.AccAddress) (bool, *common.Address) {
	...
	k.Logger(ctx).Info("IsTokenizedAccount", "taEntry", taEntry.String())
...
func (k Keeper) RedirectTokenizedAccountExcessBalances(ctx sdk.Context, account sdk.AccAddress, changedErc20s []*common.Address) error {
	logger := k.Logger(ctx)
	logger.Info("Enter RedirectTokenizedAccountExcessBalances", "receiver", account.String(), "changedBalances", changedErc20s)
	isTokenized, nft := k.IsTokenizedAccountWithValue(ctx, account)
	if !isTokenized {
		logger.Info("receiver not tokenized")
		return nil // Do nothing for non-Tokenized Accounts
	}
	logger.Info("receiver tokenized")
	// If the account IS tokenized, it MUST have its NFT registered
	if nft == nil {
		panic("discovered nil nft address in tokenized account entry")
	}
	logger.Info("Discovered TokenizedAccount->NFT entry", "account", account.String(), "nft", nft.Hex())

	logger.Info("Querying thresholds")
	thresholds, err := k.queryTokenizedAccountThresholds(ctx, *nft)
	if err != nil {
		panic("failed to query evm for tokenized account thresholds")
	}
	logger.Info("Found thresholds", "thresholds", thresholds)

	changedThresholdedErc20s := types.FindThresholdIntersection(thresholds, changedErc20s)
	logger.Info("Found thresholds + modified balances intersection", "thresholds", thresholds, "changedBalances", changedErc20s, "intersection", changedThresholdedErc20s)
	...
	for _, toRedirect := range changedThresholdedErc20s {
		...
		logger.Info("Found no pair for threshold token", "token", toRedirect.Token.Hex())
		balance := k.bankKeeper.GetBalance(ctx, account, pair.Denom)
		logger.Info("Found new balance of token", "balance", balance.String())

		balanceInExcess := balance.Amount.BigInt().Cmp(&toRedirect.Amount) > 0
		logger.Info("Checking if balance is in excess of threshold", "balance", balance.String(), "threshold", toRedirect.Amount.String(), "exceeded", balanceInExcess)
		if balanceInExcess {
			logger.Info("Redirecting balance to nft", "account", account.String(), "nft", nft.Hex(), "exceeded", balanceInExcess)
			funneled, err := k.RedirectBalanceToToken(ctx, account, *nft, balance, toRedirect.Amount)
			if err != nil {
				return err
			}
			logger.Info("Redirected to nft", "amount", funneled.String())
			funneledAmounts = append(funneledAmounts, *funneled)
		}
	}

	logger.Info("Emitting balance redirect event to log")
...
func (k Keeper) RedirectBalanceToToken(
	...
	logger := k.Logger(ctx)
	logger.Info("Enter RedirectBalanceToToken", "account", account.String(), "nft", nft.Hex(), "currBalance", currBalance.String(), "thresholdAmount", thresholdAmount.String())
	excessBalance := currBalance.Amount.Sub(sdk.NewIntFromBigInt(&thresholdAmount))
	logger.Info("Checking balance again", "excessBalance", excessBalance.String())
	...
	logger.Info("About to redirect balance via convert coin", "msg", msgConvertCoin.String())
	if err := msgConvertCoin.ValidateBasic(); err != nil {
		return nil, sdkerrors.Wrap(err, "generated invalid convert coin msg")
	}
	logger.Info("Calling convert coin")
	...
	logger.Info("Redirected balanace via convert coin")
...
func IsEthermintAccount(account authtypes.AccountI) bool {
	...
	return pk != nil && pk.Type() == ethsecp256k1.PubKey{}.Type()
}
